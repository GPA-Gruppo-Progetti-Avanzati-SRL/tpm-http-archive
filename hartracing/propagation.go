...
// Extracted from github.com/opentracing/opentracing-go@v1.2.0/propagation.go to handle the serialization over thw wire of the context.
...
type StringCarrier string

// ForeachKey conforms to the TextMapReader interface.
func (c StringCarrier) ForeachKey(handler func(key, val string) error) error {
	err := handler(HARTraceIdHeaderName, string(c))
	return err
}
